...
#Region "Methods"
    ...
    ''' <summary>
    ''' Create the product country status table.
    ''' </summary>
    ''' <param name="p_dstCertUnCertCountries"></param>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function CreateProdCountryStatusTable(ByRef p_dstCertUnCertCountries As DataSet) As DataTable
        ...
        For Each drwRow As DataRow In dtbCertCountriesWithCertification.Rows
            If Not dtbProdRegCountryStatus.Columns.Contains(drwRow(NameAid.Column.CertificationName)) Then
    ...
    ''' <summary>
    ''' Gets the product country status table.
    ''' </summary>
    ...
    Public Function GetProductCountryStatusTable(ByVal p_strBrand As String, ByVal p_strBrandLine As String, ByVal p_strRegionName As String) As DataSet
        ...
        For Each drwProdCountry As DataRow In dtbProductCountry.Rows
            ...
            If Not drwProdCountry(NameAid.Column.CountryID).Equals(DBNull.Value) Then
                Dim strCountryID = drwProdCountry(NameAid.Column.CountryID).ToString
                If drwStatus.Table.Columns.Contains(strCountryID) Then
                    drwStatus(strCountryID) = drwProdCountry(NameAid.Column.State)
                Else
                    Dim strCertificationName = drwProdCountry(NameAid.Column.CertificationName).ToString
                    If dtbCertCountriesWithCertification.Rows.Contains(strCountryID) And drwStatus.Table.Columns.Contains(strCertificationName) Then
                        drwStatus(drwProdCountry(NameAid.Column.CertificationName)) = drwProdCountry(NameAid.Column.State)
                    End If
                End If
            End If
    ...
    ''' </summary>
    ''' <param name="dtbProdRegCountryStatus"></param>
    ''' <returns></returns>
    ''' <remarks></remarks>
    ...
    ''' <summary>
    ''' Save the change for current product-country-status table
    ''' </summary>
    ''' <param name="dtbProdRegCountryStatus"></param>
    ''' <remarks></remarks>
            ...
            For Each dclProdCountry As DataColumn In drwProdCountry.Table.Columns
                ...
                And dclProdCountry.ColumnName <> "SingLoadIndex" And dclProdCountry.ColumnName <> "DualLoadIndex" And dclProdCountry.ColumnName <> "SpeedRating" Then
                    ...
                    If dclProdCountry.ColumnName <> dclProdCountry.Caption Then
                        ...
                        blnDone = Depository.Current.SaveCertificationRequest(Not (drwProdCountry(dclProdCountry) <> NameAid.MarketingStatus.Uncertified), drwProdCountry(NameAid.Column.MaterialNumber), Val(dclProdCountry.ColumnName), drwProdCountry(NameAid.Column.SKUID))
                    Else
                        Dim CertificationId As Integer
                        For Each drwCertificationGroupRow As DataRow In dtbCertificationGroupCountries.Rows
                            If dclProdCountry.ColumnName = drwCertificationGroupRow(NameAid.Column.CertificationName) Then
                                CertificationId = drwCertificationGroupRow(NameAid.Column.CertificationId)
                                Exit For
                            End If
                        Next
                        blnDone = Depository.Current.SaveCertificationGroup(Not (drwProdCountry(dclProdCountry) <> NameAid.MarketingStatus.Uncertified), drwProdCountry(NameAid.Column.MaterialNumber), CertificationId, drwProdCountry(NameAid.Column.SKUID))
                    End If
