...
Public Class TestResultsModel
    ...
    ' Added Operation as paramter for HDR SaveTestResults_NoTransaction methods

#Region "Methods"
    ''' <summary>
    ''' Determine Whether To Retrieve Product Data From TRACS or ICS Database
    ''' </summary>
    ...
    Public Function GetProductTRSectionData(ByVal p_strMatlNum As String, ByVal p_strCertificationNumber As String, _
        ...
        'jes 11/03/2012 -- changed to always retrieve product data from ICS -- I don't know why we would ever get it
        'jes 11/03/2012 -- again from product data - that would overwrite anything that is there
        'jes 11/03/2012 -- doesn't seem like you should do that.
    ...
    ''' <summary>
    ''' Retrive Product Data From ICS Database
    ''' </summary>
    ''' <param name="p_strMatlNum">Material number</param>
    ''' <param name="p_iSKUID">SKU Id</param>
    ''' <param name="p_strCertificationNumber">Certificate number</param>
    ...
    Private Function GetProductTRSectionData(ByVal p_strMatlNum As String, ByVal p_iSKUID As Integer, ByVal p_strCertificationNumber As String, ByRef p_iTireTypeId As Integer) As TRProductSectionData
        ...
        Dim objProduct As Product = MapICSDataTableToProduct(dtbProduct, p_strMatlNum, p_iSKUID, p_strCertificationNumber)
    ...
    ''' <summary>
    ''' Retrieve Product Data From TRACS
    ''' </summary>
    ''' <param name="p_strMatlNum">Material number</param>
    ''' <param name="p_iSKUID">SKU Id</param>
    ...
    ''' <returns>TRProductSectionData</returns>
    ''' <remarks></remarks>
    Private Function GetProductTRSectionData_SKUTRACS(ByVal p_strMatlNum As String, ByVal p_iSKUID As Integer, ByVal p_strCertificationNumber As String, ByRef p_iTireTypeId As Integer, ByVal p_iCertificationTypeId As Integer, ByVal p_iManufacturingLocationId As Integer) As TRProductSectionData

        'gets product data from ICS product table.
        Dim dstSKU As SKUtoICSDataset = Depository.Current.GetProductData_SKUTRACS(p_strMatlNum)


        Dim objTRTemp As TRProductSectionData = GetProductTRSectionData(p_strMatlNum, p_iSKUID, p_strCertificationNumber, p_iTireTypeId)
        Dim dstTRACS As TRACStoICSDataset = Depository.Current.GetTRACSData(p_iCertificationTypeId, p_iTireTypeId, p_strMatlNum, p_iManufacturingLocationId)

        Dim objProduct As New Product
        If Not (dstTRACS Is Nothing Or dstSKU Is Nothing) Then
            objProduct = MapSKUTRACSDataTableToProduct(dstSKU.ProductData, dstTRACS.RecentTestData, p_strMatlNum, p_iSKUID, p_strCertificationNumber)
            p_iTireTypeId = objProduct.TireTypeId
        End If
    ...
    ''' </summary>
    ''' <param name="dstTRACS"></param>
    ''' <returns>String</returns>
    ''' <remarks></remarks>
    Private Function GetRecentSerialDate(ByVal dstTRACS As TRACStoICSDataset) As String
    ...
    End Function
    ''' <summary>
    ''' Compare and get the recent date in Hdr Tables 
    ''' </summary>
    ''' <param name="hdrSerialDate"></param>
    ''' <param name="recentSerialDate"></param>
    ''' <returns>String</returns>
    ''' <remarks></remarks>
    ...
    ''' <summary>
    ''' Get Hdr Date in WWYY Format
    ''' </summary>
    ''' <param name="hdrTable"></param>
    ''' <returns>String</returns>
    ''' <remarks></remarks>
    Private Function GetHdrDateInWWYYFormat(ByVal hdrTable As DataTable) As String
        ...
        If (Not hdrTable.Rows(0)("SerialDate") Is System.DBNull.Value _
                                AndAlso Not String.IsNullOrEmpty(hdrTable.Rows(0).Item("SerialDate"))) Then
            If (IsDate(hdrTable.Rows(0)("SerialDate"))) Then
                hdrDateInWWYYFormat = GetWeekOfYearInWWYYFormat(hdrTable.Rows(0)("SerialDate"))
            Else
    ...
    ''' Get the Week and year from passed serial date.
    ''' Prefix "0" if the wwyy format contains 3 digits.
    ''' </summary>
    ...
    Private Function GetWeekOfYearInWWYYFormat(ByVal serialDate As Date) As String
        ...
        If (Not String.IsNullOrEmpty(serialDate)) Then
            calendarYear = serialDate.Year
            calendarYear = calendarYear.Substring(2, 2)
            calendarWeek = cal.GetWeekOfYear(serialDate, dfi.CalendarWeekRule, dfi.FirstDayOfWeek)
    ...
    ''' <returns>String</returns>
    ''' <remarks></remarks>
    Private Function GetRecentTestDate(ByVal dstTRACS As TRACStoICSDataset) As Date
        Dim recentDate As Date = "01/01/1901"
        ...
        If (Not dstTRACS Is Nothing AndAlso dstTRACS.Tables.Count > 0) Then
            ...
            Next
        End If
        'If (recentDate = "01/01/1900") Then
        '    recentDate = System.DBNull
        'End If
        Return recentDate
    End Function
    ''' <summary>
    ...
    Private Function GetHdrTestDate(ByVal hdrTestDate As Date, ByVal recentTestDate As Date) As Date
        ''If (String.IsNullOrEmpty(hdrSerialDate) AndAlso hdrSerialDate.Length <> 4 AndAlso Not IsNumeric(hdrSerialDate)) Then
        ''    Return String.Empty
        ''End If
        Dim recentDate As String = recentTestDate
        'If (CInt(hdrSerialDate.Substring(2, 2)) > CInt(recentSerialDate.Substring(2, 2))) Then
        '    recentDate = hdrSerialDate
        'ElseIf (CInt(hdrSerialDate.Substring(2, 2)) = CInt(recentSerialDate.Substring(2, 2))) Then
        '    If (CInt(hdrSerialDate.Substring(0, 2)) > CInt(recentSerialDate.Substring(0, 2))) Then
        ...
        If hdrTestDate > recentTestDate Then
            recentDate = hdrTestDate
        Else
            recentDate = recentTestDate
        End If
        Return recentDate
    End Function
    ...
    Private Function GetHdrDateInDateFormat(ByVal hdrTable As DataTable) As Date
        Dim hdrDateInDateFormat As Date = "01/01/1901"
        If (Not hdrTable.Rows(0)("CompletionDate") Is System.DBNull.Value _
                                AndAlso Not String.IsNullOrEmpty(hdrTable.Rows(0).Item("CompletionDate"))) Then
            If (IsDate(hdrTable.Rows(0)("CompletionDate"))) Then
                hdrDateInDateFormat = hdrTable.Rows(0).Item("CompletionDate")
            Else
    ...
    ''' <summary>
    ''' Map ICS Datasets product datatable to Product entity object
    ''' </summary>
    ''' <param name="p_dtbICSProduct"></param>
    ''' <param name="p_strMatlNum"></param>
    ''' <param name="p_iSKUID"></param>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function MapICSDataTableToProduct(ByVal p_dtbICSProduct As ICSDataSet.ProductDataDataTable, ByVal p_strMatlNum As String, ByVal p_iSKUID As Integer, ByVal p_strCertificationNumber As String)

        Dim objProduct As Product
        Dim dstTRACSProduct As New SKUtoICSDataset
        dstTRACSProduct.EnforceConstraints = False
        dstTRACSProduct.ProductData.Merge(p_dtbICSProduct)

        objProduct = MapSKUTRACSDataTableToProduct(dstTRACSProduct.ProductData, Nothing, p_strMatlNum, p_iSKUID, p_strCertificationNumber)
        ...
        drwProd = p_dtbICSProduct.Rows(0)

        objProduct.MaterialNumber = drwProd.MATL_NUM.TrimStart("0"c)
        ...
        If Not drwProd.IsMostRecentTestDateNull Then
            objProduct.MostRecentTestDate = drwProd.MostRecentTestDate
        End If
        If Not drwProd.IsSerialDateNull Then
            objProduct.SerialDate = drwProd.SerialDate
        End If
        If Not drwProd.IsPlantProducedNull Then
            objProduct.PlantProduced = drwProd.PlantProduced
        End If
        If Not drwProd.IsTireTypeIdNull Then
            objProduct.TireId = drwProd.TireTypeId
        End If
        'Added TPN
        ...
        If Not drwProd.IsSEVEREWEATHERINDNull Then
            objProduct.SevereWeatherIndicator = IIf(drwProd.SEVEREWEATHERIND.ToUpper() = "Y", True, False)
        End If
        Return objProduct
    ...
    ''' <summary>
    ''' Map SKUTRACS datasets product datatable to Product
    ''' </summary>
    ''' <param name="p_dtbProduct"></param>
    ''' <param name="p_strMatlNum"></param>
    ''' <param name="p_iSKUID"></param>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function MapSKUTRACSDataTableToProduct(ByVal p_dtbProduct As SKUtoICSDataset.ProductDataDataTable, ByVal p_dtbRecentTestData As TRACStoICSDataset.RecentTestDataDataTable, ByVal p_strMatlNum As String, ByVal p_iSKUID As Integer, ByVal p_strCertificationNumber As String)

        Dim objProduct As New Product()
        Dim drwProd As SKUtoICSDataset.ProductDataRow
        Dim drwRTD As TRACStoICSDataset.RecentTestDataRow
        ...
        End If
        If Not drwProd.IsTubelessYNNull AndAlso drwProd.TubelessYN.ToLower().Equals("y") Then
            objProduct.TubelessYN = True
        End If
        If Not drwProd.IsReinforcedYNNull AndAlso drwProd.ReinforcedYN.ToLower().Equals("y") Then
            objProduct.ReinforcedYN = True
        End If
        If Not drwProd.IsExtraLoadYNNull AndAlso drwProd.ExtraLoadYN.ToLower().Equals("y") Then
        ...
        End If
        If Not drwProd.IsMudSnowYNNull AndAlso drwProd.MudSnowYN.ToLower().Equals("y") Then
        ...
        If Not p_dtbRecentTestData Is Nothing Then
            drwRTD = p_dtbRecentTestData.Rows(0)

            Try
                If Not drwRTD.IsSerialDateNull Then
                    objProduct.SerialDate = drwRTD.SerialDate
                End If
        ...
        End If
        If Not drwProd.IsSevereWeatherIndNull Then
            objProduct.SevereWeatherIndicator = IIf(drwProd.SevereWeatherInd.Trim().ToUpper() = "Y", True, False)
        End If
        If Not drwProd.IsMFGWWYYNull Then
            objProduct.MFGWWYY = drwProd.MFGWWYY
        End If
        ...
        If p_strCertificationNumber Is Nothing Then
            ...
            'Rim width conversion from mm to inches
            'objProduct.MeaRimWidth = objProduct.MeaRimWidth / 25.4
            'Rim Diameter convert from inches to mm
            'objProduct.RimDiameter = objProduct.RimDiameter * 25.4
        End If
    ...
    Private Function MapProductToTRSectionData(ByVal p_objProduct As Product, ByVal p_blnSKUTRACSIndicator As Boolean) As TRProductSectionData
        ...
        objTRProductSectionData.MeasuringRim = p_objProduct.MeaRimWidth

        'Try
        '    Dim Week As Integer = ConvertToInteger(p_objProduct.DOTSerialNumber.Substring(p_objProduct.DOTSerialNumber.Length - 4, 2))
        '    Dim Year As Integer = ConvertToInteger(p_objProduct.DOTSerialNumber.Substring(p_objProduct.DOTSerialNumber.Length - 2, 2)) + 2000
        '    Dim dtmDateOfManufacture As DateTime = New DateTime(Year, 1, 1).AddDays(Week * 7)
        '    objTRProductSectionData.DateOfManufacture = dtmDateOfManufacture.ToShortDateString()
        'Catch exc As Exception
        '    ' Leave default value
        'End Try
        ...
        objTRProductSectionData.NominalRimDiameter = p_objProduct.RimDiameter
        ...
        objTRProductSectionData.Type = IIf(p_objProduct.TubelessYN, "Tubeless", "Tube")
        ...
        If p_objProduct.MostRecentTestDate.Year <= 1901 Then
            'p_objProduct.MostRecentTestDate = #1/1/1901#
        ...
        If String.IsNullOrEmpty(p_objProduct.TireTypeId) Then
            objTRProductSectionData.TireId = 0
        Else
            objTRProductSectionData.TireId = p_objProduct.TireTypeId
        End If
    ...
    Public Function GetTRSectionData(ByVal p_strMatlNum As String, _
        ...
        Dim dstTRACStoICSDataset As TRACSSharedDatasets.TRACStoICSDataset
        ...
        If Not String.IsNullOrEmpty(p_strCertificateNumber) Then
            dstICSDataset = Depository.Current.GetTestResultData(p_strMatlNum, p_iSKUId, p_strCertificateNumber, p_intCertificateNumberID, p_intCertificationTypeId)
        ...
        Else
            intCertType = p_intCertificationTypeId
            ...
            'retList = MapSKUTRACSDataSetToTRSectionData(dstTRACStoICSDataset, p_strMatlNum, p_iSKUId, p_intCertificateNumberID, p_intCertificationTypeId)
    ...
    Public Function GetRequestedTests(ByVal p_strMatlNum As String, ByVal p_iSKUId As Integer, ByVal p_iCertificateNumberId As Integer, _
        ...
        Dim dstTRACStoICSDataset As New TRACSSharedDatasets.TRACStoICSDataset
        Dim ostTestReq As New TRACSSharedDatasets.ClientRequest
        Dim retList As New List(Of Object)

        For Each Row As DataRow In p_dtbClientRequest.Rows
            Dim ostTestReqRow As TRACSSharedDatasets.ClientRequest.TestRequestRow = ostTestReq.TestRequest.NewRow
            ostTestReqRow.ProjectNum = Row.Item("ProjectNum")
            ostTestReqRow.TireNum = Row.Item("TireNum")

            'Changed as per PRJ3617 SAP Interface to International Certification System (OPQ.I.8265)Remediation. 
            ostTestReqRow.Operation = Row.Item("Operation")
            ostTestReqRow.TestSpec = Row.Item("TestSpec")

            ostTestReq.TestRequest.AddTestRequestRow(ostTestReqRow)
        Next

        dstTRACStoICSDataset = Depository.Current.GetRequestedTests(p_iCertificationTypeId, p_iTireTypeId, ostTestReq)
        'dstTRACStoICSDataset.WriteXml("c:\tracs1.xml", XmlWriteMode.WriteSchema)
        ...
        retList = MapSKUTRACSDataSetToTRSectionData(dstTRACStoICSDataset, p_strMatlNum, p_iSKUId, p_iCertificateNumberId, p_iCertificationTypeId)
        'retList.WriteXmlSchema("c:\tracs2.xsd")
    ...
    Private Function MapICSDataSetToTRSectionData(ByVal dstICSDataset As ICSDataSet, ByVal p_strMatlNum As String, _
        ...
        Dim objMeasure As Measure = MapICSDataTableToMeasure(dstICSDataset.MeasureHdr, dstICSDataset.MeasureDtl, p_intCertificationTypeId, p_strMatlNum, p_iSKUId, p_intCertificateNumberID)
        Dim objTreadwear As Treadwear = MapICSDataTableToTreadwear(dstICSDataset.TreadWearHdr, dstICSDataset.TreadWearDtl, p_intCertificationTypeId, p_strMatlNum, p_iSKUId, p_intCertificateNumberID)
        Dim objPlunger As Plunger = MapICSDataTableToPlunger(dstICSDataset.PlungerHdr, dstICSDataset.PlungerDtl, p_intCertificationTypeId, p_strMatlNum, p_iSKUId, p_intCertificateNumberID)
        Dim objBeadUnSeat As BeadUnSeat = MapICSDataTableToBeadUnSeat(dstICSDataset.BeadUnseatHdr, dstICSDataset.BeadUnseatDtl, p_intCertificationTypeId, p_strMatlNum, p_iSKUId, p_intCertificateNumberID)
        Dim objEndurance As Endurance = MapICSDataTableToEndurance(dstICSDataset.EnduranceHdr, dstICSDataset.EnduranceDtl, p_intCertificationTypeId, p_strMatlNum, p_iSKUId, p_intCertificateNumberID)
        Dim objHighSpeed As HighSpeed = MapICSDataTableToHighSpeed(dstICSDataset.HighSpeedHdr, dstICSDataset.HighSpeedDtl, dstICSDataset.SpeedTestDetail, p_intCertificationTypeId, p_strMatlNum, p_iSKUId, p_intCertificateNumberID)
        Dim objSound As Sound = MapICSDataTableToSound(dstICSDataset.SoundHdr, dstICSDataset.SoundDetail, p_intCertificationTypeId, p_strMatlNum, p_iSKUId, p_intCertificateNumberID)
        Dim objWetGrip As WetGrip = MapICSDataTableToWetGrip(dstICSDataset.WetGripHdr, dstICSDataset.WetGripDetail, p_intCertificationTypeId, p_strMatlNum, p_iSKUId, p_intCertificateNumberID)

        retList = MapEntityToTRSectionData(objMeasure, objTreadwear, objPlunger, objBeadUnSeat, objEndurance, objHighSpeed, objSound, objWetGrip, p_intCertificationTypeId)

        Return retList

    End Function

    Private Function MapSKUTRACSDataSetToTRSectionData(ByVal dstTRACStoICSDataset As TRACSSharedDatasets.TRACStoICSDataset, _
        ...
        If (Not dstTRACStoICSDataset Is Nothing And dstTRACStoICSDataset.Tables.Count > 0) Then
            ...
            Dim objMeasure As Measure = MapTRACSDataTableToMeasure(dstTRACStoICSDataset.MeasureHdr, dstTRACStoICSDataset.MeasureDtl, p_intCertificationTypeId, p_strMatlNum, p_iSKUId, p_intCertificateNumberID)
            Dim objTreadwear As Treadwear = MapTRACSDataTableToTreadwear(dstTRACStoICSDataset.TreadWearHdr, dstTRACStoICSDataset.TreadWearDtl, p_intCertificationTypeId, p_strMatlNum, p_iSKUId, p_intCertificateNumberID)
            Dim objPlunger As Plunger = MapTRACSDataTableToPlunger(dstTRACStoICSDataset.PlungerHdr, dstTRACStoICSDataset.PlungerDtl, p_intCertificationTypeId, p_strMatlNum, p_iSKUId, p_intCertificateNumberID)
            Dim objBeadUnSeat As BeadUnSeat = MapTRACSDataTableToBeadUnSeat(dstTRACStoICSDataset.BeadUnseatHdr, dstTRACStoICSDataset.BeadUnseatDtl, p_intCertificationTypeId, p_strMatlNum, p_iSKUId, p_intCertificateNumberID)
            Dim objEndurance As Endurance = MapTRACSDataTableToEndurance(dstTRACStoICSDataset.EnduranceHdr, dstTRACStoICSDataset.EnduranceDtl, p_intCertificationTypeId, p_strMatlNum, p_iSKUId, p_intCertificateNumberID)
            Dim objHighSpeed As HighSpeed = MapTRACSDataTableToHighSpeed(dstTRACStoICSDataset.HighSpeedHdr, dstTRACStoICSDataset.HighSpeedDtl, p_intCertificationTypeId, p_strMatlNum, p_iSKUId, p_intCertificateNumberID)
            retList = MapEntityToTRSectionData(objMeasure, objTreadwear, objPlunger, objBeadUnSeat, objEndurance, objHighSpeed, Nothing, Nothing, p_intCertificationTypeId)
    ...
    Private Function MapICSDataTableToMeasure(ByVal p_dtbICSMeasureHdr As ICSDataSet.MeasureHdrDataTable, _
        ...
        Dim dtbTRACSMeasureHdr As New TRACStoICSDataset.MeasureHdrDataTable
        Dim dtbTRACSMeasureDtl As New TRACStoICSDataset.MeasureDtlDataTable
        ...
        drwMeasureHdr = p_dtbICSMeasureHdr.Rows(0)

        If Not drwMeasureHdr.IsMATL_NUMNull Then
            objMeasure.MaterialNumber = drwMeasureHdr.MATL_NUM.TrimStart("0"c)
        End If
        ...
        If Not drwMeasureHdr.IsSTARTINFLATIONPRESSURENull Then
            objMeasure.StartInfPressure = drwMeasureHdr.STARTINFLATIONPRESSURE
        End If

        If Not drwMeasureHdr.IsENDINFLATIONPRESSURENull Then
            objMeasure.EndInfPressure = drwMeasureHdr.ENDINFLATIONPRESSURE
        End If
        ...
        If Not drwMeasureHdr.IsNOMINALWIDTHPASSFAILNull Then
            objMeasure.NominalWidthPassYN = IIf(drwMeasureHdr.NOMINALWIDTHPASSFAIL = "y", True, False)
        End If
        ...
        End If
        'If Not drwMeasureHdr.IsMaxOverallWidthNull Then
        '    objMeasure.MaxOverallWidth = drwMeasureHdr.MaxOverallWidth
        'End If
        ...
        If Not drwMeasureHdr.IsOVERALLWIDTHPASSFAILNull Then
            objMeasure.OverallWidthPassYN = IIf(drwMeasureHdr.OVERALLWIDTHPASSFAIL = "n", False, True)
        End If
        If Not drwMeasureHdr.IsOVERALLDIAMETERPASSFAILNull Then
            objMeasure.OverallDiameterPassYN = IIf(drwMeasureHdr.OVERALLDIAMETERPASSFAIL = "y", True, False)
        End If
        ...
        If Not drwMeasureHdr.IsELONGATION1Null Then
            objMeasure.Elongation1 = drwMeasureHdr.ELONGATION1
        End If
        If Not drwMeasureHdr.IsELONGATION2Null Then
            objMeasure.Elongation2 = drwMeasureHdr.ELONGATION2
        End If
    ...
    ''' <summary>
    ''' Maps the data table to measure.
    ''' </summary>
    ...
    ''' <returns></returns>
    Private Function MapTRACSDataTableToMeasure(ByVal p_dtbTRACSMeasureHdr As TRACStoICSDataset.MeasureHdrDataTable, _
                                            ByVal p_dtbTRACSMeasureDtl As TRACStoICSDataset.MeasureDtlDataTable, _
                                            ByVal p_iCertificationTypeId As Integer, ByVal p_strMatlNum As String, _
                                            ByVal p_iSKUId As Integer, ByVal p_intCertificateNumberID As Integer) As Measure

        Dim objMeasure As New Measure()
        Dim drwMeasure As TRACStoICSDataset.MeasureHdrRow
        Dim drwMeasureDetail As TRACStoICSDataset.MeasureDtlRow
        ...
        'Measure Header
        drwMeasure = p_dtbTRACSMeasureHdr.Rows(0)
        ...
        End If
        objMeasure.TestSpec = IIf(drwMeasure.IsTestSpecNull, String.Empty, drwMeasure.TestSpec)
        ...
        If p_intCertificateNumberID = 0 Then
            ...
            'Commented out all metric conversion statements.

            '1 pound per square inch = 0.0689475729 bar
            'objMeasure.InflationPressure = Math.Round(objMeasure.InflationPressure * 0.0689475729, 1)
            objMeasure.InflationPressure = Math.Round(objMeasure.InflationPressure * 0.01, 1)
            objMeasure.RimWidth = Math.Round(objMeasure.RimWidth * 25.4)
            'objMeasure.Diameter = Math.Round(objMeasure.Diameter * 25.4)
            'objMeasure.AvgSectionWidth = Math.Round(objMeasure.AvgSectionWidth * 25.4)
            'objMeasure.AvgOverallWidth = Math.Round(objMeasure.AvgOverallWidth * 25.4)
            'objMeasure.MinSizeFactor = Math.Round(objMeasure.MinSizeFactor * 25.4)
            'objMeasure.ActSizeFactor = Math.Round(objMeasure.ActSizeFactor * 25.4)

            'For Each objDetail As MeasureDetail In objMeasure.MeasureDetails
            '    objDetail.SectionWidth = Math.Round(objDetail.SectionWidth * 25.4)
            '    objDetail.OverallWidth = Math.Round(objDetail.OverallWidth * 25.4)
            'Next
        End If

        If Not drwMeasure.IsGTSPECNull Then
            objMeasure.GTSpecMaterialNumber = drwMeasure.GTSPEC.TrimStart("0"c)
        End If
    ...
    Private Function MapICSDataTableToTreadwear(ByVal p_dtbICSTreadwearHdr As ICSDataSet.TreadWearHdrDataTable, _
        ...
        Dim dtbTRACSTreadwearHdr As New TRACStoICSDataset.TreadWearHdrDataTable
        Dim dtbTRACSTreadwearDtl As New TRACStoICSDataset.TreadWearDtlDataTable
        ...
        drwTreadwearHdr = p_dtbICSTreadwearHdr.Rows(0)
        ...
        If Not drwTreadwearHdr.IsPassYNNull Then
            If drwTreadwearHdr.PassYN.ToUpper = "Y" Then
        ...
        If Not drwTreadwearHdr.IsMATL_NUMNull Then
            objTreadwear.MaterialNumber = drwTreadwearHdr.MATL_NUM.TrimStart("0"c)
        End If

        If Not drwTreadwearHdr.IsGTSPECNull Then
            objTreadwear.GTSpecMaterialNumber = drwTreadwearHdr.GTSPEC.TrimStart("0"c)
        End If
    ...
    Private Function MapTRACSDataTableToTreadwear(ByVal p_dtbTreadwearHdr As TRACStoICSDataset.TreadWearHdrDataTable, _
                                                ByVal p_dtbTreadwearDtl As TRACStoICSDataset.TreadWearDtlDataTable, _
                                                ByVal p_iCertificationTypeId As Integer, ByVal p_strMatlNum As String, _
                                                ByVal p_iSKUId As Integer, ByVal p_intCertificateNumberID As Integer) As Treadwear

        Dim objTreadwear As New Treadwear()
        Dim drwTreadwear As TRACStoICSDataset.TreadWearHdrRow
        Dim drwTreadwearDetail As TRACStoICSDataset.TreadWearDtlRow
        ...
        drwTreadwear = p_dtbTreadwearHdr.Rows(0)
        ...
        End If
        objTreadwear.TestSpec = IIf(drwTreadwear.IsTestSpecNull, String.Empty, drwTreadwear.TestSpec)
        ...
        Try
            If Not drwTreadwear.IsCompletionDateNull Then
                objTreadwear.CompletionDate = drwTreadwear.CompletionDate
            End If
        Catch exc As Exception
            ' Leave default value
        End Try
        ...
        If Not drwTreadwear.IsPassYNNull Then
            objTreadwear.PassYN = IIf(drwTreadwear.PassYN.ToLower() = "y", True, False)
        End If
        ...
        If Not drwTreadwear.IsGTSPECNull Then
            objTreadwear.GTSpecMaterialNumber = drwTreadwear.GTSPEC.TrimStart("0"c)
        End If
        ...
        'Conversion
        If p_intCertificateNumberID = 0 Then
            If objTreadwear.CertificationTypeId <> Depository.Current.GetCertificationTypeID("ECE3054") Then
                For Each objDetail As TreadwearDetail In objTreadwear.TreadwearDetails
                    objDetail.WearBarHeight = Math.Round(objDetail.WearBarHeight * 25.4)
                Next
            Else
                For Each objDetail As TreadwearDetail In objTreadwear.TreadwearDetails
                    objDetail.WearBarHeight = Math.Round(objDetail.WearBarHeight * 25.4, 1)
                Next
            End If
        End If
    ...
    Private Function MapICSDataTableToPlunger(ByVal p_dtbICSPlungerHdr As ICSDataSet.PlungerHdrDataTable, _
        ...
        Dim dtbTRACSPlungerHdr As New TRACStoICSDataset.PlungerHdrDataTable
        Dim dtbTRACSPlungerDtl As New TRACStoICSDataset.PlungerDtlDataTable
        ...
        Dim dst As New TRACStoICSDataset
        ...
        drwPlungerHdr = p_dtbICSPlungerHdr.Rows(0)

        If Not drwPlungerHdr.IsMATL_NUMNull Then
            objPlunger.MaterialNumber = drwPlungerHdr.MATL_NUM.TrimStart("0"c)
        End If

        If Not drwPlungerHdr.IsGTSPECNull Then
            objPlunger.GTSpecMaterialNumber = drwPlungerHdr.GTSPEC.TrimStart("0"c)
        End If
    ...
    'Defect 10034: Modified by offshore on 26NOV2012. Commented out all metric conversion statements.
    Private Function MapTRACSDataTableToPlunger(ByVal p_dtbPlungerHdr As TRACStoICSDataset.PlungerHdrDataTable, _
                                            ByVal p_dtbPlungerDtl As TRACStoICSDataset.PlungerDtlDataTable, _
                                            ByVal p_iCertificationTypeId As Integer, ByVal p_strMatlNum As String, _
                                            ByVal p_iSKUId As Integer, ByVal p_intCertificateNumberID As Integer) As Plunger

        Dim objPlunger As New Plunger()
        Dim drwPlunger As TRACStoICSDataset.PlungerHdrRow
        Dim drwPlungerDetail As TRACStoICSDataset.PlungerDtlRow
        ...
        drwPlunger = p_dtbPlungerHdr.Rows(0)
        ...
        End If
        objPlunger.TestSpec = IIf(drwPlunger.IsTestSpecNull, String.Empty, drwPlunger.TestSpec)
        ...
        Try
            If Not drwPlunger.IsCompletionDateNull Then
                objPlunger.CompletionDate = drwPlunger.CompletionDate
            End If
        Catch exc As Exception
            ' Leave default value
        End Try
        ...
        If Not drwPlunger.IsPassYNNull Then
            objPlunger.PassYN = IIf(drwPlunger.PassYN.ToLower() = "y", True, False)
        End If
        ...
        'Conversion
        'If p_intCertificateNumberID = 0 Then
        '    objPlunger.MinPlunger = Math.Round(objPlunger.MinPlunger * 0.112984829)

        '    objPlunger.AVGBreakingEnergy = Math.Round(objPlunger.AVGBreakingEnergy * 0.112984829)

        '    'Convert from inch pounds to newtons meter
        '    For Each objDetail As PlungerDetail In objPlunger.PlungerDetails
        '        objDetail.BreakingEnergy = Math.Round(objDetail.BreakingEnergy * 0.112984829)
        '    Next
        'End If

        If Not drwPlunger.IsGTSPECNull Then
            objPlunger.GTSpecMaterialNumber = drwPlunger.GTSPEC.TrimStart("0"c)
        End If
    ...
    Private Function MapICSDataTableToBeadUnSeat(ByVal p_dtbICSBeadUnSeatHdr As ICSDataSet.BeadUnseatHdrDataTable, _
        ...
        Dim dtbTRACSBeadUnSeatHdr As New TRACStoICSDataset.BeadUnseatHdrDataTable
        Dim dtbTRACSBeadUnSeatDtl As New TRACStoICSDataset.BeadUnseatDtlDataTable
        ...
        drwBeadUnSeatHdr = p_dtbICSBeadUnSeatHdr.Rows(0)

        If Not drwBeadUnSeatHdr.IsTestPassFailNull Then
            objBeadUnSeat.TestPassFail = IIf(drwBeadUnSeatHdr.TestPassFail = "y", True, False)
        End If

        If Not drwBeadUnSeatHdr.IsMATL_NUMNull Then
            objBeadUnSeat.MaterialNumber = drwBeadUnSeatHdr.MATL_NUM.TrimStart("0"c)
        End If

        If Not drwBeadUnSeatHdr.IsGTSPECNull Then
            objBeadUnSeat.GTSpecMaterialNumber = drwBeadUnSeatHdr.GTSPEC.TrimStart("0"c)
        End If
    ...
    Private Function MapTRACSDataTableToBeadUnSeat(ByVal p_dtbBeadUnSeatHdr As TRACStoICSDataset.BeadUnseatHdrDataTable, _
                                                ByVal p_dtbBeadUnSeatDtl As TRACStoICSDataset.BeadUnseatDtlDataTable, _
                                                ByVal p_iCertificationTypeId As Integer, ByVal p_strMatlNum As String, _
                                                ByVal p_iSKUId As Integer, ByVal p_intCertificateNumberID As Integer) As BeadUnSeat

        Dim objBeadUnSeat As New BeadUnSeat()
        Dim drwBeadUnSeat As TRACStoICSDataset.BeadUnseatHdrRow
        Dim drwBeadUnSeatDetail As TRACStoICSDataset.BeadUnseatDtlRow
        ...
        drwBeadUnSeat = p_dtbBeadUnSeatHdr.Rows(0)
        ...
        End If
        objBeadUnSeat.TestSpec = IIf(drwBeadUnSeat.IsTestSpecNull, String.Empty, drwBeadUnSeat.TestSpec)
        ...
        Try
            If Not drwBeadUnSeat.IsCompletionDateNull Then
                objBeadUnSeat.CompletionDate = drwBeadUnSeat.CompletionDate
            End If
        Catch exc As Exception
            ' Leave default value
        End Try
        ...
        If Not drwBeadUnSeat.IsPassYNNull Then
            objBeadUnSeat.PassYN = IIf(drwBeadUnSeat.PassYN.ToLower() = "y", True, False)
            objBeadUnSeat.TestPassFail = IIf(drwBeadUnSeat.PassYN.ToLower() = "y", True, False) 'Not used so just set the same.
        End If
        ...
        If p_intCertificateNumberID = 0 Then
            ...
            'objBeadUnSeat.LowestUnSeatValue = Math.Round(objBeadUnSeat.LowestUnSeatValue * 4.44822162)
            'objBeadUnSeat.MinBeadUnseat = Math.Round(objBeadUnSeat.MinBeadUnseat * 4.44822162)
        End If

        If Not drwBeadUnSeat.IsGTSPECNull Then
            objBeadUnSeat.GTSpecMaterialNumber = drwBeadUnSeat.GTSPEC.TrimStart("0"c)
        End If
    ...
    ''' <summary>
    ''' Map Data From ICS Database Endurance Header And Detail Tables To Endurance Entity 
    ''' </summary>
    ...
    ''' <returns></returns>
    ''' <remarks>Endurance (Entity)</remarks>
    ''' 
    Private Function MapICSDataTableToEndurance(ByVal p_dtbICSEnduranceHdr As ICSDataSet.EnduranceHdrDataTable, ByVal p_dtbICSEnduranceDtl As ICSDataSet.EnduranceDtlDataTable, ByVal p_iCertificationTypeId As Integer, ByVal p_strMatlNum As String, ByVal p_iSKUId As Integer, ByVal p_intCertificateNumberID As Integer) As Endurance
        ...
        Dim dtbTRACSEnduranceHdr As New TRACStoICSDataset.EnduranceHdrDataTable
        Dim dtbTRACSEnduranceDtl As New TRACStoICSDataset.EnduranceDtlDataTable
        ...
        drwEnduranceHdr = p_dtbICSEnduranceHdr.Rows(0)

        If Not drwEnduranceHdr.IsMATL_NUMNull Then
            objEndurance.MaterialNumber = drwEnduranceHdr.MATL_NUM.TrimStart("0"c)
        End If
        ...
        If Not drwEnduranceHdr.IsPRECONDTEMPNull Then
            objEndurance.PrecondStartTemp = drwEnduranceHdr.PRECONDTEMP
        End If
        If Not drwEnduranceHdr.IsINFLATIONPRESSUREREADJUSTEDNull Then
            objEndurance.InflationPressureReadjusted = drwEnduranceHdr.INFLATIONPRESSUREREADJUSTED
        End If
        ...
        If Not drwEnduranceHdr.IsRESULTPASSFAILNull Then
            objEndurance.ResultPassFail = IIf(drwEnduranceHdr.RESULTPASSFAIL = "y", True, False)
        End If
        If Not drwEnduranceHdr.IsPassYNNull Then
            objEndurance.PassYN = IIf(drwEnduranceHdr.PassYN = "y", True, False)
            objEndurance.EnduranceTestPassYN = IIf(drwEnduranceHdr.PassYN = "y", True, False)
        End If
        ...
        If Not drwEnduranceHdr.IsFINALJUDGEMENTNull Then
            objEndurance.FinalJudgement = IIf(drwEnduranceHdr.FINALJUDGEMENT = "y", True, False)
        End If
        ...
        If Not drwEnduranceHdr.IsLOWPRESSURESTARTINFLATIONNull Then
            objEndurance.LowInfStartInflation = drwEnduranceHdr.LOWPRESSURESTARTINFLATION
        End If

        'Low Pressure End Inflation
        If Not drwEnduranceHdr.IsLOWPRESSUREENDINFLATIONNull Then
            objEndurance.LowInfEndInflation = drwEnduranceHdr.LOWPRESSUREENDINFLATION
        End If
        ...
        If Not drwEnduranceHdr.IsGTSPECNull Then
            objEndurance.GTSpecMaterialNumber = drwEnduranceHdr.GTSPEC.TrimStart("0"c)
        End If
    ...
    'Defect 10034: Modified by offshore on 26NOV2012. Commented out all metric conversion statements.
    Private Function MapTRACSDataTableToEndurance(ByVal p_dtbEnduranceHdr As TRACStoICSDataset.EnduranceHdrDataTable, _
                                                ByVal p_dtbEnduranceDtl As TRACStoICSDataset.EnduranceDtlDataTable, _
                                                ByVal p_iCertificationTypeId As Integer, ByVal p_strMatlNum As String, _
                                                ByVal p_iSKUId As Integer, ByVal p_intCertificateNumberID As Integer) As Endurance

        Dim objEndurance As New Endurance
        Dim drwEndurance As TRACStoICSDataset.EnduranceHdrRow
        Dim drwEnduranceDetail As TRACStoICSDataset.EnduranceDtlRow
        ...
        drwEndurance = p_dtbEnduranceHdr.Rows(0)
        ...
        If Not drwEndurance.IsPassYNNull Then
            objEndurance.EnduranceTestPassYN = IIf(drwEndurance.PassYN.ToLower() = "y", True, False)
            objEndurance.ResultPassFail = IIf(drwEndurance.PassYN.ToLower() = "y", True, False)
        End If
        ...
        For Each drwEnduranceDetail In p_dtbEnduranceDtl.Rows
            ...
            Try
                If Not drwEnduranceDetail.IsStepCompletionDateNull Then
                    objEnduranceDetail.StepCompletionDate = drwEnduranceDetail.StepCompletionDate
                End If
            Catch exc As Exception
                ' Leave default value
            End Try
        ...
        If p_intCertificateNumberID = 0 Then
            ...
            'objEndurance.FinalTemp = Math.Round((objEndurance.FinalTemp - 32) * 5 / 9)
            'objEndurance.LowInfEndTemp = Math.Round((objEndurance.LowInfEndTemp - 32) * 5 / 9)
            'objEndurance.PostcondEndTemp = Math.Round((objEndurance.PostcondEndTemp - 32) * 5 / 9)
            'objEndurance.PrecondEndTemp = Math.Round((objEndurance.PrecondEndTemp - 32) * 5 / 9)
            'objEndurance.PrecondStartTemp = Math.Round((objEndurance.PrecondStartTemp - 32) * 5 / 9)

            'Modified by offshore on 26NOV2012.

            If objEndurance.CertificationTypeId <> Depository.Current.GetCertificationTypeID("ECE3054") Then

                '1 Psi = 6.89475729 KPA
                'If objEndurance.CertificationTypeId = NameAid.Certification.GSO Or objEndurance.CertificationTypeId = NameAid.Certification.NOM Then
                '    'Convert to KPA
                '    objEndurance.AfterInflation = objEndurance.AfterInflation * 6.89475729
                '    objEndurance.BeforeInflation = objEndurance.BeforeInflation * 6.89475729
                '    objEndurance.FinalInflation = objEndurance.FinalInflation * 6.89475729
                '    objEndurance.InflationPressure = objEndurance.InflationPressure * 6.89475729
                '    objEndurance.LowInfStartInflation = objEndurance.LowInfStartInflation * 6.89475729
                '    objEndurance.LowInfEndInflation = objEndurance.LowInfEndInflation * 6.89475729

                '    'Approved Substitutions
                '    objEndurance.InflationPressure = Math.Round(Depository.Current.GetApprovedSubstitution(objEndurance.CertificationTypeId, "PASSENGER ENDURANCE INFLATION", objEndurance.InflationPressure, objEndurance.SKUID))
                '    objEndurance.FinalInflation = Math.Round(Depository.Current.GetApprovedSubstitution(objEndurance.CertificationTypeId, "PASSENGER ENDURANCE INFLATION", objEndurance.FinalInflation, objEndurance.SKUID))
                '    objEndurance.AfterInflation = Math.Round(Depository.Current.GetApprovedSubstitution(objEndurance.CertificationTypeId, "PASSENGER ENDURANCE INFLATION", objEndurance.AfterInflation, objEndurance.SKUID))
                '    objEndurance.BeforeInflation = Math.Round(Depository.Current.GetApprovedSubstitution(objEndurance.CertificationTypeId, "PASSENGER ENDURANCE INFLATION", objEndurance.BeforeInflation, objEndurance.SKUID))
                '    objEndurance.LowInfStartInflation = Math.Round(Depository.Current.GetApprovedSubstitution(objEndurance.CertificationTypeId, "PASSENGER ENDURANCE INFLATION", objEndurance.LowInfStartInflation, objEndurance.SKUID))
                '    objEndurance.LowInfEndInflation = Math.Round(Depository.Current.GetApprovedSubstitution(objEndurance.CertificationTypeId, "PASSENGER ENDURANCE INFLATION", objEndurance.LowInfEndInflation, objEndurance.SKUID))
                'Else
                '    objEndurance.AfterInflation = Math.Round(objEndurance.AfterInflation * 6.89475729)
                '    objEndurance.BeforeInflation = Math.Round(objEndurance.BeforeInflation * 6.89475729)
                '    objEndurance.FinalInflation = Math.Round(objEndurance.FinalInflation * 6.89475729)
                '    objEndurance.InflationPressure = Math.Round(objEndurance.InflationPressure * 6.89475729)
                '    objEndurance.LowInfStartInflation = Math.Round(objEndurance.LowInfStartInflation * 6.89475729)
                '    objEndurance.LowInfEndInflation = Math.Round(objEndurance.LowInfEndInflation * 6.89475729)
                'End If

                If objEndurance.CertificationTypeId = Depository.Current.GetCertificationTypeID("GSO") Or objEndurance.CertificationTypeId = Depository.Current.GetCertificationTypeID("NOM") Then
                    'Approved Substitutions
                    objEndurance.InflationPressure = Math.Round(Depository.Current.GetApprovedSubstitution(objEndurance.CertificationTypeId, "PASSENGER ENDURANCE INFLATION", objEndurance.InflationPressure, objEndurance.SKUID))
                    objEndurance.FinalInflation = Math.Round(Depository.Current.GetApprovedSubstitution(objEndurance.CertificationTypeId, "PASSENGER ENDURANCE INFLATION", objEndurance.FinalInflation, objEndurance.SKUID))
                    objEndurance.AfterInflation = Math.Round(Depository.Current.GetApprovedSubstitution(objEndurance.CertificationTypeId, "PASSENGER ENDURANCE INFLATION", objEndurance.AfterInflation, objEndurance.SKUID))
            ...
            Else
                '1 pound per square inch = 0.0689475729 bar
                'objEndurance.AfterInflation = Math.Round(objEndurance.AfterInflation * 0.0689475729, 1)
                'objEndurance.BeforeInflation = Math.Round(objEndurance.BeforeInflation * 0.0689475729, 1)
                'objEndurance.FinalInflation = Math.Round(objEndurance.FinalInflation * 0.0689475729, 1)
                'objEndurance.InflationPressure = Math.Round(objEndurance.InflationPressure * 0.0689475729, 1)
                'objEndurance.LowInfStartInflation = Math.Round(objEndurance.LowInfStartInflation * 0.0689475729, 1)
                'objEndurance.LowInfEndInflation = Math.Round(objEndurance.LowInfEndInflation * 0.0689475729, 1)

                objEndurance.AfterInflation = Math.Round(objEndurance.AfterInflation * 0.01, 1)
                objEndurance.BeforeInflation = Math.Round(objEndurance.BeforeInflation * 0.01, 1)
                objEndurance.FinalInflation = Math.Round(objEndurance.FinalInflation * 0.01, 1)
                objEndurance.InflationPressure = Math.Round(objEndurance.InflationPressure * 0.01, 1)
                objEndurance.LowInfStartInflation = Math.Round(objEndurance.LowInfStartInflation * 0.01, 1)
                objEndurance.LowInfEndInflation = Math.Round(objEndurance.LowInfEndInflation * 0.01, 1)
            End If

            'Modified by offshore on 26NOV2012.

            '1 inches = 25.4 mm
            'objEndurance.AfterDiameter = Math.Round(objEndurance.AfterDiameter * 25.4)
            'objEndurance.BeforeDiameter = Math.Round(objEndurance.BeforeDiameter * 25.4)           
            'objEndurance.RimDiameter = Math.Round(objEndurance.RimDiameter * 25.4)
            ''NOTE: Not consistant to Product (use inch)
            objEndurance.RimWidth = Math.Round(objEndurance.RimWidth * 25.4)

            '1 mile = 1.609344 kilometers
            'objEndurance.FinalDistance = Math.Round(objEndurance.FinalDistance * 1.609344)

            For Each objDetail As EnduranceDetail In objEndurance.EnduranceDetails
                'Modified by offshore on 26NOV2012.

                'convert degrees fahrenheit to degrees celsius C=((F-32)*5)/9
                'If objDetail.AmbTemp <> -1 Then
                '    objDetail.AmbTemp = Math.Round((objDetail.AmbTemp - 32) * 5 / 9)
                'End If

                'If objEndurance.CertificationTypeId <> NameAid.Certification.ECE3054 Then
                '    objDetail.InfPressure = Math.Round(objDetail.InfPressure * 6.89475729)
                'Else
                '    '1 pound per square inch = 0.0689475729 bar
                '    objDetail.InfPressure = Math.Round(objDetail.InfPressure * 0.0689475729, 1)
                'End If

                If objEndurance.CertificationTypeId = Depository.Current.GetCertificationTypeID("ECE3054") Then
                    objDetail.InfPressure = Math.Round(objDetail.InfPressure * 0.01, 1)
                End If
                ...
                If objEndurance.CertificationTypeId <> Depository.Current.GetCertificationTypeID("ECE3054") Then
                    ...
                    'If objEndurance.CertificationTypeId = NameAid.Certification.GSO Or objEndurance.CertificationTypeId = NameAid.Certification.NOM Then
                    '    'Convert to KPA
                    '    objDetail.InfPressure = objDetail.InfPressure * 6.89475729

                    '    'Approved Substitutions
                    '    objDetail.InfPressure = Math.Round(Depository.Current.GetApprovedSubstitution(objEndurance.CertificationTypeId, "PASSENGER ENDURANCE INFLATION", objDetail.InfPressure, objEndurance.SKUID))
                    'Else
                    '    objDetail.InfPressure = Math.Round(objDetail.InfPressure * 6.89475729)
                    'End If
                ...
                '1 mile = 1.609344 kilometers
                'objDetail.TotMiles = Math.Round(objDetail.TotMiles * 1.609344)

                '1 pound = 0.45359237 kilograms
                'objDetail.Load = Math.Round(objDetail.Load * 0.45359237)

                '1 mile hr = 1.609344 kilometers hr
                'If objEndurance.CertificationTypeId = NameAid.Certification.NOM Then
                '    objDetail.Speed = objDetail.Speed * 1.609344
                '    'Approved Substitutions
                '    objDetail.Speed = Math.Round(Depository.Current.GetApprovedSubstitution(objEndurance.CertificationTypeId, "PASSENGER WHEEL TEST SPEED", objDetail.Speed, objEndurance.SKUID))
                'Else
                '    objDetail.Speed = Math.Round(objDetail.Speed * 1.609344)
                'End If

                If objEndurance.CertificationTypeId = Depository.Current.GetCertificationTypeID("NOM") Then
                    'Approved Substitutions
                    objDetail.Speed = Math.Round(Depository.Current.GetApprovedSubstitution(objEndurance.CertificationTypeId, "PASSENGER WHEEL TEST SPEED", objDetail.Speed, objEndurance.SKUID))
                End If
            Next
        ...
        If Not drwEndurance.IsGTSPECNull Then
            objEndurance.GTSpecMaterialNumber = drwEndurance.GTSPEC.TrimStart("0"c)
        End If
    ...
    Private Function MapICSDataTableToHighSpeed(ByVal p_dtbICSHighSpeedHdr As ICSDataSet.HighSpeedHdrDataTable, _
        ...
        Dim dtbTRACSHighSpeedHdr As New TRACStoICSDataset.HighSpeedHdrDataTable
        Dim dtbTRACSHighSpeedDtl As New TRACStoICSDataset.HighSpeedDtlDataTable
        ...
        drwHighSpeedHdr = p_dtbICSHighSpeedHdr.Rows(0)

        If Not drwHighSpeedHdr.IsMATL_NUMNull Then
            objHighSpeed.MaterialNumber = drwHighSpeedHdr.MATL_NUM.TrimStart("0"c)
        End If
        ...
        If Not drwHighSpeedHdr.IsSPEEDTTESTPASSFAILNull Then
            objHighSpeed.SpeedTestPassYN = IIf(drwHighSpeedHdr.SPEEDTTESTPASSFAIL = "y", True, False)
        End If
        ...
        If Not drwHighSpeedHdr.IsFINALJUDGEMENTNull Then
            objHighSpeed.FinalJudgement = IIf(drwHighSpeedHdr.FINALJUDGEMENT = "y", True, False)
        End If
        ...
        If Not drwHighSpeedHdr.IsGTSPECNull Then
            objHighSpeed.GTSpecMaterialNumber = drwHighSpeedHdr.GTSPEC.TrimStart("0"c)
        End If
    ...
    Private Function MapICSDataTableToHighSpeedSpeedTestDetail(ByVal p_dtbICSSpeedTestDetail As ICSDataSet.SpeedTestDetailDataTable) As List(Of SpeedTestDetail)
        ...
        For Each drwSpeedTestDetail In p_dtbICSSpeedTestDetail.Rows
            If Not drwSpeedTestDetail.IsIterationNull Then
                ...
                objSpeedTestDetail.Iteration = drwSpeedTestDetail.Iteration
    ...
    'Defect 10034: Modified by offshore on 26NOV2012. Commented out all metric conversion statements.
    Private Function MapTRACSDataTableToHighSpeed(ByVal p_dtbHighSpeedHdr As TRACStoICSDataset.HighSpeedHdrDataTable, _
                                                ByVal p_dtbHighSpeedDtl As TRACStoICSDataset.HighSpeedDtlDataTable, _
                                                ByVal p_iCertificationTypeId As Integer, ByVal p_strMatlNum As String, _
                                                ByVal p_iSKUId As Integer, ByVal p_intCertificateNumberID As Integer) As HighSpeed

        Dim objHighSpeed As New HighSpeed
        Dim drwHighSpeed As TRACStoICSDataset.HighSpeedHdrRow
        Dim drwHighSpeedDetail As TRACStoICSDataset.HighSpeedDtlRow
        ...
        drwHighSpeed = p_dtbHighSpeedHdr.Rows(0)
        ...
        If Not drwHighSpeed.IsPassYNNull Then
            objHighSpeed.DurationTestPassYN = IIf(drwHighSpeed.PassYN.ToLower() = "y", True, False)
        End If
        ...
        If Not drwHighSpeed.IsGTSPECNull Then
            objHighSpeed.GTSpecMaterialNumber = drwHighSpeed.GTSPEC.TrimStart("0"c)
        End If
        ...
        For Each drwHighSpeedDetail In p_dtbHighSpeedDtl.Rows
            ...
            objHighSpeedDetail.Iteration = Iteration
            ...
            Try
                If Not drwHighSpeedDetail.IsStepCompletionDateNull Then
                    objHighSpeedDetail.StepCompletionDate = drwHighSpeedDetail.StepCompletionDate
                End If
            Catch exc As Exception
                ' Leave default value
            End Try
        ...
        If p_intCertificateNumberID = 0 Then
            ...
            'objHighSpeed.FinalTemp = Math.Round((objHighSpeed.FinalTemp - 32) * 5 / 9)
            'objHighSpeed.PostcondEndTemp = Math.Round((objHighSpeed.PostcondEndTemp - 32) * 5 / 9)
            'objHighSpeed.PrecondEndTemp = Math.Round((objHighSpeed.PrecondEndTemp - 32) * 5 / 9)
            'objHighSpeed.PrecondStartTemp = Math.Round((objHighSpeed.PrecondStartTemp - 32) * 5 / 9)

            'Modified by offshore on 26NOV2012.

            If objHighSpeed.CertificationTypeId <> Depository.Current.GetCertificationTypeID("ECE3054") Then

                '1 Psi = 6.89475729 KPA
                'If objHighSpeed.CertificationTypeId = NameAid.Certification.GSO Or objHighSpeed.CertificationTypeId = NameAid.Certification.NOM Then
                '    'Convert to KPA
                '    objHighSpeed.AfterInflation = objHighSpeed.AfterInflation * 6.89475729
                '    objHighSpeed.BeforeInflation = objHighSpeed.BeforeInflation * 6.89475729
                '    objHighSpeed.FinalInflation = objHighSpeed.FinalInflation * 6.89475729
                '    objHighSpeed.InflationPressure = objHighSpeed.InflationPressure * 6.89475729

                '    'Approved Substitutions
                '    objHighSpeed.AfterInflation = Math.Round(Depository.Current.GetApprovedSubstitution(objHighSpeed.CertificationTypeId, "PASSENGER HIGHSPEED INFLATION", objHighSpeed.AfterInflation, objHighSpeed.SKUID))
                '    objHighSpeed.BeforeInflation = Math.Round(Depository.Current.GetApprovedSubstitution(objHighSpeed.CertificationTypeId, "PASSENGER HIGHSPEED INFLATION", objHighSpeed.BeforeInflation, objHighSpeed.SKUID))
                '    objHighSpeed.FinalInflation = Math.Round(Depository.Current.GetApprovedSubstitution(objHighSpeed.CertificationTypeId, "PASSENGER HIGHSPEED INFLATION", objHighSpeed.FinalInflation, objHighSpeed.SKUID))
                '    objHighSpeed.InflationPressure = Math.Round(Depository.Current.GetApprovedSubstitution(objHighSpeed.CertificationTypeId, "PASSENGER HIGHSPEED INFLATION", objHighSpeed.InflationPressure, objHighSpeed.SKUID))
                'Else
                '    objHighSpeed.AfterInflation = Math.Round(objHighSpeed.AfterInflation * 6.89475729)
                '    objHighSpeed.BeforeInflation = Math.Round(objHighSpeed.BeforeInflation * 6.89475729)
                '    objHighSpeed.FinalInflation = Math.Round(objHighSpeed.FinalInflation * 6.89475729)
                '    objHighSpeed.InflationPressure = Math.Round(objHighSpeed.InflationPressure * 6.89475729)
                'End If

                If objHighSpeed.CertificationTypeId = Depository.Current.GetCertificationTypeID("GSO") Or objHighSpeed.CertificationTypeId = Depository.Current.GetCertificationTypeID("NOM") Then
                    'Approved Substitutions
                    objHighSpeed.AfterInflation = Math.Round(Depository.Current.GetApprovedSubstitution(objHighSpeed.CertificationTypeId, "PASSENGER HIGHSPEED INFLATION", objHighSpeed.AfterInflation, objHighSpeed.SKUID))
                    objHighSpeed.BeforeInflation = Math.Round(Depository.Current.GetApprovedSubstitution(objHighSpeed.CertificationTypeId, "PASSENGER HIGHSPEED INFLATION", objHighSpeed.BeforeInflation, objHighSpeed.SKUID))
                    objHighSpeed.FinalInflation = Math.Round(Depository.Current.GetApprovedSubstitution(objHighSpeed.CertificationTypeId, "PASSENGER HIGHSPEED INFLATION", objHighSpeed.FinalInflation, objHighSpeed.SKUID))
            ...
            Else
                ...
                '1 pound per square inch = 0.0689475729 bar
                'objHighSpeed.AfterInflation = Math.Round(objHighSpeed.AfterInflation * 0.0689475729, 1)
                'objHighSpeed.BeforeInflation = Math.Round(objHighSpeed.BeforeInflation * 0.0689475729, 1)
                'objHighSpeed.FinalInflation = Math.Round(objHighSpeed.FinalInflation * 0.0689475729, 1)
                'objHighSpeed.InflationPressure = Math.Round(objHighSpeed.InflationPressure * 0.0689475729, 1)

                objHighSpeed.AfterInflation = Math.Round(objHighSpeed.AfterInflation * 0.01, 1)
                objHighSpeed.BeforeInflation = Math.Round(objHighSpeed.BeforeInflation * 0.01, 1)
                objHighSpeed.FinalInflation = Math.Round(objHighSpeed.FinalInflation * 0.01, 1)
                objHighSpeed.InflationPressure = Math.Round(objHighSpeed.InflationPressure * 0.01, 1)
            End If

            'Modified by offshore on 26NOV2012.

            '1 inches = 25.4 mm
            'objHighSpeed.AfterDiameter = Math.Round(objHighSpeed.AfterDiameter * 25.4)
            'objHighSpeed.BeforeDiameter = Math.Round(objHighSpeed.BeforeDiameter * 25.4)
            'objHighSpeed.RimDiameter = Math.Round(objHighSpeed.RimDiameter * 25.4)
            ' ''NOTE: not consistance to Product section (using inches instead of MM)
            objHighSpeed.RimWidth = Math.Round(objHighSpeed.RimWidth * 25.4)

            'Modified by offshore on 26NOV2012.

            '1 mile = 1.609344 kilometers
            'objHighSpeed.FinalDistance = Math.Round(objHighSpeed.FinalDistance * 1.609344)
            ...
            For Each objDetail As HighSpeedDetail In objHighSpeed.HighSpeedDetails
                ...
                If objHighSpeed.CertificationTypeId <> Depository.Current.GetCertificationTypeID("ECE3054") Then
                    ...
                    '    'Approved Substitutions
                    '    objDetail.InfPressure = Math.Round(Depository.Current.GetApprovedSubstitution(objHighSpeed.CertificationTypeId, "PASSENGER ENDURANCE INFLATION", objDetail.InfPressure, objHighSpeed.SKUID))
                    'Else
                    '    objDetail.InfPressure = Math.Round(objDetail.InfPressure * 6.89475729)
                    'End If
                ...
                '1 mile hr = 1.609344 kilometers hr
                'If objHighSpeed.CertificationTypeId = NameAid.Certification.NOM Then
                '    objDetail.Speed = objDetail.Speed * 1.609344
                '    'Approved Substitutions
                '    objDetail.Speed = Math.Round(Depository.Current.GetApprovedSubstitution(objHighSpeed.CertificationTypeId, "PASSENGER WHEEL TEST SPEED", objDetail.Speed, objHighSpeed.SKUID))
                'Else
                '    objDetail.Speed = Math.Round(objDetail.Speed * 1.609344)
                'End If

                If objHighSpeed.CertificationTypeId = Depository.Current.GetCertificationTypeID("NOM") Then
                    'Approved Substitutions
                    objDetail.Speed = Math.Round(Depository.Current.GetApprovedSubstitution(objHighSpeed.CertificationTypeId, "PASSENGER WHEEL TEST SPEED", objDetail.Speed, objHighSpeed.SKUID))
                End If
    ...
    Private Function MapICSDataTableToSound(ByVal p_dtbSoundHdr As ICSDataSet.SoundHdrDataTable, _
        ...
        If Not drwSound.IsTIRENUMBERNull Then
            objSound.TireNumber = drwSound.TIRENUMBER
        End If
        'Added Operation  as per PRJ3617 SAP Interface to International Certification System (OPQ.I.8265)Remediation 
        ...
        If Not drwSound.IsSOUNDIDNull Then
            objSound.SoundID = drwSound.SOUNDID
        End If
        ...
        If Not drwSound.IsTIRELOADINDEX_REARLNull Then
            objSound.TireLoadIndex_RearL = drwSound.IsTIRELOADINDEX_REARLNull
        End If
        If Not drwSound.IsTIRELOADINDEX_REARRNull Then
            objSound.TireLoadIndex_RearR = drwSound.IsTIRELOADINDEX_REARRNull
        End If
        ...
        If Not drwSound.IsGTSPECNull Then
            objSound.GTSpecMaterialNumber = drwSound.GTSPEC.TrimStart("0"c)
        End If
        ...
        For Each drwSoundDetail In p_dtbSoundDtl.Rows
            ...
            objSoundDetail.SoundID = drwSoundDetail.SOUNDID

            objSoundDetail.Iteration = drwSoundDetail.ITERATION
    ...
    Private Function MapICSDataTableToWetGrip(ByVal p_dtbWetGripHdr As ICSDataSet.WetGripHdrDataTable, _
        ...
        drwWetGrip = p_dtbWetGripHdr.Rows(0)
        ...
        If Not drwWetGrip.IsTIRENUMBERNull Then
            objWetGrip.TireNumber = drwWetGrip.TIRENUMBER
        End If
        ...
        If Not drwWetGrip.IsWETGRIPIDNull Then
            objWetGrip.WetGripID = drwWetGrip.WETGRIPID
        End If
        ...
        If Not drwWetGrip.IsGTSPECNull Then
            objWetGrip.GTSpecMaterialNumber = drwWetGrip.GTSPEC.TrimStart("0"c)
        End If
        ...
        For Each drwWetGripDetail In p_dtbWetGripDtl.Rows
            ...
            objWetGripDetail.WetGripID = drwWetGripDetail.WETGRIPID

            objWetGripDetail.Iteration = drwWetGripDetail.ITERATION
    ...
    Private Function MapEntitiesToTRMeasurementSectionData(ByVal p_objMeasure As Measure, ByVal p_objTreadwear As Treadwear, ByVal p_objPlunger As Plunger, ByVal p_objBeadUnSeat As BeadUnSeat, ByVal p_intCertificationTypeId As Integer) As TRMeasurementSectionData
        ...
        objTRMeasurementSectionData.StartTime = p_objMeasure.MountTime.ToString("hh:mm tt")
        objTRMeasurementSectionData.EndTime = p_objMeasure.CompletionDate.ToString("hh:mm tt")
        ...
        objTRMeasurementSectionData.TotalTime = intHour.ToString() + "Hr" + intMin.ToString() + "Min"
        ...
        objTRMeasurementSectionData.OverallTolerance = p_objMeasure.DiameterTolerance
        ...
        If intPlungerTotal > 0 Then
            p_objPlunger.AVGBreakingEnergy = (intPlungerTotal / intDetailsCount)
            objTRMeasurementSectionData.PlungerAverage = p_objPlunger.AVGBreakingEnergy.ToString()
        Else
            objTRMeasurementSectionData.PlungerAverage = p_objPlunger.AVGBreakingEnergy.ToString()
        End If
        ...
        Else
            ...
            If intUnseatForceTotal > 0 Then
                If p_intCertificationTypeId = Depository.Current.GetCertificationTypeID("NOM") Then
                    intUnseatForceTotal = intUnseatForceTotal - p_objBeadUnSeat.BeadUnSeatDetails(0).UnSeatForce
                    p_objBeadUnSeat.MinBeadUnseat = (intUnseatForceTotal / (intDetailsCount - 1)).ToString
                    p_objBeadUnSeat.LowestUnSeatValue = (intUnseatForceTotal / (intDetailsCount - 1)).ToString
                    objTRMeasurementSectionData.BeadUnseatTestKN = p_objBeadUnSeat.MinBeadUnseat
                Else
                    p_objBeadUnSeat.MinBeadUnseat = (intUnseatForceTotal / intDetailsCount).ToString
                    p_objBeadUnSeat.LowestUnSeatValue = (intUnseatForceTotal / intDetailsCount).ToString
                    objTRMeasurementSectionData.BeadUnseatTestKN = p_objBeadUnSeat.MinBeadUnseat
                End If
            Else
                objTRMeasurementSectionData.BeadUnseatTestKN = p_objBeadUnSeat.MinBeadUnseat
        ...
        '        If Not String.IsNullOrEmpty(p_objMeasure.CertificateNumber) Then
    ...
    Private Function MapEnduranceBeforeSectionData(ByVal p_objEndurance As Endurance) As TREnduranceTestGeneralBeforeSectionData
        ...
        objTREnduranceBeforeSectionData.EnduranceCircumferenceBefore = p_objEndurance.BeforeDiameter * 3.1416.ToString()
    ...
    Private Function MapEnduranceSectionData(ByVal p_objEndurance As Endurance) As TREnduranceSectionData
        ...
        For Each enduranceDetail As EnduranceDetail In p_objEndurance.EnduranceDetails
            ...
            Else
                ...
                Dim HourField As System.Reflection.FieldInfo = objTREnduranceSectionData.GetType().GetField("EnduranceHours" + (enduranceDetail.Iteration).ToString())
        ...
        If p_objEndurance.EnduranceXHour > 0 Then
            objTREnduranceSectionData.EnduranceXHours = p_objEndurance.EnduranceXHour
        Else
    ...
    Private Function MapHighSpeedSectionData(ByVal p_objHighSpeed As HighSpeed) As TRHighSpeedSectionData
        ...
        For i As Integer = 0 To intDetailsCount - 1
            ...
            If objHighSpeedDetail.StepCompletionDate <> DateTime.MinValue Then
                ...
                Dim TimeField As System.Reflection.FieldInfo = objTRHighSpeedSectionData.GetType().GetField("HighSpeedDurationTime" + objHighSpeedDetail.Iteration.ToString())
        ...
        'Show the range for the first teststep in highspeed duration test table
        ...
        objTRHighSpeedSectionData.HighSpeedTotalTime = p_objHighSpeed.SpeedTotalTime
        objTRHighSpeedSectionData.HighSpeedMaxSpeed = p_objHighSpeed.MaxSpeed
        objTRHighSpeedSectionData.HighSpeedMaxLoad = p_objHighSpeed.MaxLoad
        objTRHighSpeedSectionData.HighSpeedWheelSpeedRPM = p_objHighSpeed.WheelSpeedRPM
        objTRHighSpeedSectionData.HighSpeedWheelSpeedKMH = p_objHighSpeed.WheelSpeedKMH

        objTRHighSpeedSectionData.HighSpeedDurationPassYN = p_objHighSpeed.DurationTestPassYN
        objTRHighSpeedSectionData.HighSpeedInfPressureBefore = p_objHighSpeed.InflationPressure
        objTRHighSpeedSectionData.HighSpeedInfPressureAfter = p_objHighSpeed.FinalInflation
        objTRHighSpeedSectionData.HighSpeedTempAfter = p_objHighSpeed.FinalTemp

        intFieldCount = 9
        intDetailsCount = p_objHighSpeed.SpeedTestDetails.Count()
        ''Debug.Assert(intDetailsCount <= intFieldCount, "SpeedTestDetails.Count is greater than form FieldCount. Excess data is ignored.")
        intDetailsCount = Math.Min(intDetailsCount, intFieldCount)
    ...
    Private Function SaveTestResults_NoTransaction(ByVal p_objProduct As Product) As NameAid.SaveResult
        ...
        Dim chrTubelessYN As String = IIf(p_objProduct.TubelessYN, "Y", "N")
        Dim chrReinforcedYN As String = IIf(p_objProduct.ReinforcedYN, "Y", "N")
        Dim chrExtraLoadYN As String = IIf(p_objProduct.ExtraLoadYN, "Y", "N")
        Dim chrMudSnowYN As String = IIf(p_objProduct.MudSnow, "Y", "N")
        Dim chrRegroovableYN As String = IIf(p_objProduct.RegroovableInd, "Y", "N")
        Dim chrSevereWeatherIndicator As String = IIf(p_objProduct.SevereWeatherIndicator, "Y", "N")
        If Not String.IsNullOrEmpty(p_objProduct.TireId) Then
            p_objProduct.TireTypeId = p_objProduct.TireId
        End If
    ...
    Private Function SaveTestResults_NoTransaction(ByVal p_objMeasure As Measure) As NameAid.SaveResult
        ...
        Dim enumSaveResult As NameAid.SaveResult = NameAid.SaveResult.SaveError
        Dim strNominalWithPassYN As String = IIf(p_objMeasure.NominalWidthPassYN.Equals(True), "y", "n")
        Dim strOVERALLWIDTHPASSFAIL As String = IIf(p_objMeasure.OverallWidthPassYN.Equals(True), "y", "n")
        Dim strOVERALLDIAMETERPASSFAIL As String = IIf(p_objMeasure.OverallDiameterPassYN.Equals(True), "y", "n")
    ...
    Private Function SaveTestResults_NoTransaction(ByVal p_objPlunger As Plunger) As NameAid.SaveResult
        ...
        Dim p_strPassYN = IIf(p_objPlunger.PassYN.Equals(True), "y", "n")
    ...
    Private Function SaveTestResults_NoTransaction(ByVal p_objTreadwear As Treadwear) As NameAid.SaveResult
        ...
        Dim p_strPassYN = IIf(p_objTreadwear.PassYN.Equals(True), "y", "n")
    ...
    Private Function SaveTestResults_NoTransaction(ByVal p_objBeadUnSeat As BeadUnSeat) As NameAid.SaveResult
        ...
        Dim p_strPassYN As String = IIf(p_objBeadUnSeat.PassYN.Equals(True), "y", "n")
        Dim p_TestPassFail As String = IIf(p_objBeadUnSeat.TestPassFail.Equals(True), "y", "n")
    ...
    Private Function SaveTestResults_NoTransaction(ByVal p_objEndurance As Endurance) As NameAid.SaveResult
        ...
        Dim p_strPassYN = IIf(p_objEndurance.EnduranceTestPassYN.Equals(True), "y", "n")
        Dim p_strResultPassFail = IIf(p_objEndurance.ResultPassFail.Equals(True), "y", "n")
        Dim p_strFinalJudgement = IIf(p_objEndurance.FinalJudgement.Equals(True), "y", "n")
        ...
        enumSaveResult = Depository.Current.SaveEndurance(p_intEnduranceId, _
                                                     ...
                                                     p_objEndurance.LowInfEndTemp, _
        ...
        For Each detail As EnduranceDetail In p_objEndurance.EnduranceDetails
            ...
            enumSaveResult = Depository.Current.SaveEnduranceDetail(detail.TestStep, _
                                                               ...
                                                               detail.SetInflation, _
                                                               detail.AmbTemp, _
                                                               detail.InfPressure, _
    ...
    Private Function SaveTestResults_NoTransaction(ByVal p_objHighSpeed As HighSpeed) As NameAid.SaveResult
        ...
        Dim p_strPassYN As String = IIf(p_objHighSpeed.DurationTestPassYN.Equals(True), "Y", "N")
        Dim p_strFinalJudg As String = IIf(p_objHighSpeed.FinalJudgement.Equals(True), "Y", "N")
        Dim p_strSpeedPassFail As String = IIf(p_objHighSpeed.SpeedTestPassYN.Equals(True), "Y", "N")
    ...
    ''' <summary>
    ''' Map TRProductSectionData to product entity class
    ''' </summary>
    ''' <param name="p_objTRProductSectionData"></param>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Function MapTRProductSectionDataToProduct(ByVal p_objTRProductSectionData As TRProductSectionData, _
        ...
        If Not String.IsNullOrEmpty(p_objTRProductSectionData.PlyRatingNumber) Then
            Select Case p_objTRProductSectionData.PlyRatingNumber
                ...
                Case "24"
                    objProduct.LoadRange = "N"
                Case Else
                    objProduct.LoadRange = String.Empty
            End Select
        End If
        If Not String.IsNullOrEmpty(p_objTRProductSectionData.IndicationTubeless) Then
            objProduct.TubelessYN = p_objTRProductSectionData.IndicationTubeless
        End If
        If Not String.IsNullOrEmpty(p_objTRProductSectionData.IndicationReinforced) Then
            objProduct.ReinforcedYN = p_objTRProductSectionData.IndicationReinforced
        End If
        If Not String.IsNullOrEmpty(p_objTRProductSectionData.IndicationExtraLoad) Then
        ...
        'objProduct.SerialDate
        'p_objTRProductSectionData.TireSerialNumber
        ...
        End If
        'If Not String.IsNullOrEmpty(p_objTRProductSectionData.MS) Then
        '    objProduct.MudSnowYN = IIf(p_objTRProductSectionData.MS = "Yes", True, False)
        'End If
        If Not String.IsNullOrEmpty(p_objTRProductSectionData.Regroovable) Then
        ...
        End If
        'If Not String.IsNullOrEmpty(p_objTRProductSectionData.Type) Then
        '    objProduct.TubelessYN = IIf(p_objTRProductSectionData.Type = "Tubeless", True, False)
        'End If
        ...
        If Not String.IsNullOrEmpty(p_objTRProductSectionData.MudSnow) Then
            objProduct.MudSnow = p_objTRProductSectionData.MudSnow
        End If

        If Not String.IsNullOrEmpty(p_objTRProductSectionData.SevereWeatherIndicator) Then
    ...
    Private Function MapProjectSectionData(ByVal p_objMeasure As Measure, ByVal p_objPlunger As Plunger, _
        ...
        objProjectSectionData.HighSpeedTireNumber = p_objHighSpeed.TireNumber
        ...
        If p_objSound IsNot Nothing Then
            ...
            objProjectSectionData.SoundTireNumber = p_objSound.TireNumber
        ...
        If p_objWetGrip IsNot Nothing Then
            ...
            objProjectSectionData.WetGripTireNumber = p_objWetGrip.TireNumber
    ...
    'Added Operation as per PRJ3617 SAP Interface to International Certification System (OPQ.I.8265)Remediation 
    ''' <summary>
    ''' Maps the Test Result section Data to Measure Entity
    ''' </summary>
    ...
    Public Function MapTRMeasurementSectionDataToMeasure(ByVal p_objTRMeasurement As TRMeasurementSectionData, _
        ...
        If Not p_objTRMeasurement.OriginalMeasure Is Nothing Then
            ...
            'objMeasure.DotSerialNumber = p_objTRMeasurement.OriginalMeasure.DotSerialNumber
    ...
    ''' <summary>
    ''' Maps the Test Result section Data to Measure Detail entity
    ''' </summary>
    ''' <param name="p_objTRMeasurement"></param>
    ''' <returns></returns>
    ''' <remarks></remarks>
    ...
    End Function
    ' Maps the TR measurement section dta to tread wear.
    ...
    ''' <summary>
    ''' Maps the TR measurement section dta to treadwear detail.
    ''' </summary>
    ''' <param name="p_objTRMeasurement"></param>
    ''' <returns></returns>
    ''' <remarks></remarks>
    ...
    'Added Operation as per PRJ3617 SAP Interface to International Certification System (OPQ.I.8265)Remediation 
    ''' <summary>
    ''' Maps the TR measurement section dta to bead un seat.
    ''' </summary>
    ''' <param name="p_objTRMeasurement"></param>
    ''' <param name="p_objProject"></param>
    ''' <param name="p_intCertificateNumberID"></param>
    ...
    ''' <summary>
    ''' Map TestResults measurement section data
    ''' </summary>
    ''' <param name="p_objTRMeasurement"></param>
    ''' <returns></returns>
    ''' <remarks></remarks>
    ...
    Private Function MapTREnduranceSectionDataToEnduranceDetail(ByVal p_objTREndData As TREnduranceSectionData) As List(Of EnduranceDetail)
        ...
        If Not String.IsNullOrEmpty(p_objTREndData.EnduranceDate2) Then
            ...
            End If
            'If Not p_objTREndData.OriginalEndurance Is Nothing Then
            '    objEnduranceDetail.Speed = p_objTREndData.OriginalEndurance.EnduranceDetails(2).Speed
            'Else
            '    objEnduranceDetail.Speed = 0
            'End If
        ...
        If Not String.IsNullOrEmpty(p_objTREndData.EnduranceDate3) Then
            ...
            End If
            'If Not p_objTREndData.OriginalEndurance Is Nothing Then
            '    objEnduranceDetail.Speed = p_objTREndData.OriginalEndurance.EnduranceDetails(3).Speed
            'Else
            '    objEnduranceDetail.Speed = 0
            'End If
        ...
        If Not String.IsNullOrEmpty(p_objTREndData.EnduranceDate4) Then
            ...
            End If
            'If Not p_objTREndData.OriginalEndurance Is Nothing Then
            '    objEnduranceDetail.Speed = p_objTREndData.OriginalEndurance.EnduranceDetails(4).Speed
            'Else
            '    objEnduranceDetail.Speed = 0
            'End If
    ...
    Private Function MapTRHighSpeedSectionDataToHighSpeedDetail(ByVal p_objTRHSData As TRHighSpeedSectionData) As List(Of HighSpeedDetail)
        ...
        If Not String.IsNullOrEmpty(p_objTRHSData.HighSpeedDurationStep0) Then
            Dim tempString As String() = p_objTRHSData.HighSpeedDurationStep0.Split("-")
    ...
    Public Function MapTRSoundWetSectionDataToSound(ByVal p_objTRSWData As TRSoundWetSectionData, _
        ...
        If Not String.IsNullOrEmpty(p_objTRSWData.SoundDateOfTrackCertification) Then
            objSound.DateTrackCertifToISO = p_objTRSWData.SoundDateOfTrackCertification
        End If
    ...
    Public Function MapTRSoundWetSectionDataToWetGrip(ByVal p_objTRSWData As TRSoundWetSectionData, _
        ...
        If Not String.IsNullOrEmpty(p_objTRSWData.WetDateOfTest) Then
            objWetGrip.DateOfTest = p_objTRSWData.WetDateOfTest
        End If
